  ...
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="120px">
      ...
      <el-form-item label="" prop="tagId">
        <el-input v-model="dataForm.tagId" placeholder=""></el-input>
      </el-form-item>
      <el-form-item label="" prop="operatedate">
        <el-input v-model="dataForm.operatedate" placeholder=""></el-input>
      </el-form-item>
    </el-form>
  ...
  data () {
    return {
      ...
      dataRule: {
        ...
        operatedate: [{ required: true, message: '不能为空', trigger: 'blur' }]
      }
    }
  },
  methods: {
    init (id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/admin/sys/category/info/${this.dataForm.id}`),
            method: 'get',
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/admin/sys/category/${!this.dataForm.id ? 'save' : 'update'}`),
